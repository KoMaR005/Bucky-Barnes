...
╚════════
  ...
  {"quickReplyButton": {"displayText": "Владелец 👦","id": "owner"},},
  {"quickReplyButton": {"displayText": "Статус Бот ⌚","id": `ping`}}
  ]
  ichi.sendButtonImg(m.chat, menu, global.ownerName, global.thumb, but)
  }
  break
case 'donasi': case 'sewa': case 'инфобот': {
...
💰 500/месяц
💰 1000/постоянный`
  let but = [{"quickReplyButton": {"displayText": "Владелец 👦","id": "owner"}}]
...
case 'владелец': {
  ...
  break
case 'ping': case 'botstatus': case 'статусбот': case 'speed': case 'tes': {
...
case 'bcgc': case 'bcgroup': {
  ...
  m.reply('Успешно Трансляция')
  }
  break
case 'bc': case 'трансляция': case 'bcall': {
  if (!isOwner && !m.key.fromMe) return m.reply(mess.botOwner)
  if (!text) throw `Text mana?\n\nExample : ${prefix + command} ${global.ownerName}`
...
case 'вступить': {
  ...
  if (!isUrl(args[0]) && !args[0].includes('whatsapp.com')) throw 'Ссылка недействительна!'
  m.reply(mess.wait)
...
case 'покинул': {
  if (!isOwner) return m.reply(mess.botOwner)
  await ichi.groupLeave(m.chat).then((res) => m.reply('До скорой встречи‘‹\nТы к нам заходи если что')).catch((err) => m.reply('Fitur Error ❎'))
  }
  break
case 'заблокировать': {
  if (!isOwner) return m.reply(mess.botOwner)
...
case 'публичный': {
  if (!isOwner && !m.key.fromMe) return m.reply(mess.botOwner)
  ichi.public = true
  m.reply('Успешное изменение режима бота на общедоступный')
  }
  break
case 'личный': {
  if (!isOwner && !m.key.fromMe) return m.reply(mess.botOwner)
  ichi.self = false
  m.reply('Успех в изменении режима бота на собственный')
  }
  break
case 'eval': {
  if (!isOwner && !m.key.fromMe) return m.reply(mess.botOwner)
...
case 'антиссылка':
  ...
  let buttonsantilink = [
  { buttonId: `${command} вкл`, buttonText: { displayText: 'Enable' }, type: 1 },
  { buttonId: `${command} выкл`, buttonText: { displayText: 'Disable' }, type: 1 }
  ]
...
case 'снятьадмина': {
  ...
  break
case 'сменитьимя': case 'setsubject': {
  if (!m.isGroup) return m.reply(mess.group)
  if (!isBotAdmins) return m.reply(mess.botAdmin)
  if (!isAdmins) return m.reply(mess.admin)
  if (!text) throw 'Text ?'
...
case 'setdesc': case 'setdesk': {
  ...
  break
case 'изображениегруппы': case 'setppgrup': case 'setppgc': {
  if (!m.isGroup) return m.reply(mess.group)
  if (!isAdmins) return m.reply(mess.admin)
...
case 'внимание': {
  ...
  if (!isBotAdmins) return m.reply(mess.botAdmin)
  if (!isAdmins) return m.reply(mess.admin)
let teks = `*👥 Вызов всех администраторов*
...
case 'группа': {
  ...
  if (!isAdmins) return m.reply(mess.admin)
  if (args[0] === 'close'){
  await ichi.groupSettingUpdate(m.chat, 'announcement').then((res) => m.reply(`Успешное закрытие группы`)).catch((err) => m.reply(jsonformat(err)))
  } else if (args[0] === 'open'){
  await ichi.groupSettingUpdate(m.chat, 'not_announcement').then((res) => m.reply(`Успешное открытие группы`)).catch((err) => m.reply(jsonformat(err)))
  } else {
  let buttonsgroup = [
  { buttonId: `${command} открыть`, buttonText: { displayText: 'Open' }, type: 1 },
  { buttonId: `${command} закрыть`, buttonText: { displayText: 'Close' }, type: 1 }
  ]
  await ichi.sendButtonText(m.chat, buttonsgroup, `Mode ${command} 🕊️`, `Пожалуйста, нажмите кнопку ниже, если кнопка не появляется, введите ${command} открыть/закрыть`, m)
  }
  }
  break
case 'изменитьинформацию': {
  if (!m.isGroup) return m.reply(mess.group)
  if (!isBotAdmins) return m.reply(mess.botAdmin)
  if (!isAdmins) return m.reply(mess.admin)
  if (args[0] === 'open'){
  await ichi.groupSettingUpdate(m.chat, 'открыто').then((res) => m.reply(`Успешно открыто Редактировать информацию о группе`)).catch((err) => m.reply(jsonformat(err)))
  } else if (args[0] === 'close'){
  await ichi.groupSettingUpdate(m.chat, 'закрыто').then((res) => m.reply(`Успешно закрыть Изменить информацию о группе`)).catch((err) => m.reply(jsonformat(err)))
  } else {
  let buttonsinfo = [
  { buttonId: `${command} открыть`, buttonText: { displayText: 'Open' }, type: 1 },
  { buttonId: `${command} закрыть`, buttonText: { displayText: 'Close' }, type: 1 }
...
case 'толькоизображение': case 'toimg': {
