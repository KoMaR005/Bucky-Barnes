...
let authorname = message.key.fromMe ? 'Я/СЕБЯ' : client.contacts[sender] ? client.contacts[sender].vname || client.contacts[sender].notify || sender.split('@')[0] || undefined : sender.split('@')[0]

if (isGroup){
  if (isCmd) console.log(color('[ CMD ]', 'yellow'), color(moment(t * 1000).format('ДД/ММ/ГГ ЧЧ:мм:сс'), 'green'), command,color('from', 'yellow'), authorname, color('in'), groupName)
...
if (!isGroup){
  if (isCmd) console.log(color('[ CMD ]', 'yellow'), color(moment(t * 1000).format('ДД/ММ/ГГ ЧЧ:мм:сс'), 'green'), command,color('from', 'yellow'), authorname)
